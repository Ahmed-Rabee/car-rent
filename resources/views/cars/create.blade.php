...
@push('scripts')
  ...
  <script type="text/javascript">
    ...
    $(document).ready(function () {
      ...
      const car_gallery = document.querySelector('#car-gallery');
      if (car_gallery) {
        const myDropzoneMulti = new Dropzone(car_gallery, {
          previewTemplate: previewTemplate,
          url: "/file-upload", // Replace with your upload URL
          acceptedFiles: "image/png, image/jpeg, image/jpg, image/webp",
          dictInvalidFileType: "Only images (PNG, JPEG, JPG, WEBP) are allowed.",
          parallelUploads: 12,
          maxFiles: 12, // Allow only one file
          maxFilesize: 3,
          addRemoveLinks: true,
          dictRemoveFile: "حذف الصورة",
          init: function () {
            const car_gallery_Instance = this;
            car_gallery_Instance.on("addedfile", function () {
              if (car_gallery_Instance.files.length === 1) {
                $("#car-gallery").addClass("dz-upload-disabled");
              }
            });
            car_gallery_Instance.on("removedfile", function () {
              $("#car-gallery").removeClass("dz-upload-disabled");
            });
          },
        });
      };
